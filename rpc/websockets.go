...
func (s *websocketsServer) readLoop(wsConn *wsConn) {
	for {
		...
		if err != nil {
			//20211220-xmm 关闭订阅
			wsConn.mux.Lock()
			for _, id := range wsConn.subIds {
				if !s.api.unsubscribe(id) {
					s.logger.Error("unsubscribe fail ", "id", string(id))
				}
			}
			wsConn.mux.Unlock()
			_ = wsConn.Close()
		...
		if method == "eth_subscribe" {
			...
			wsConn.subIds = append(wsConn.subIds, id)
			...
			subId := rpc.ID(ids[0].(string))
			ok = s.api.unsubscribe(subId)
			res := &SubscriptionResponseJSON{
				Jsonrpc: "2.0",
				ID:      connID,
				Result:  ok,
			...
			if ok {
				wsConn.mux.Lock()
				for index, id := range wsConn.subIds {
					if subId == id {
						wsConn.subIds = append(wsConn.subIds[:index], wsConn.subIds[index+1:]...)
					}
				}
				wsConn.mux.Unlock()
			}
...
func (api *pubSubAPI) unsubscribe(id rpc.ID) bool {
	...
	err := api.events.UnSubscribe(wsSub.sub)
	if err != nil {
		api.logger.Error("UnSubscribe fail", "error", err.Error())
		return false
	}
