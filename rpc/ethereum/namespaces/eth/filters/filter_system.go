...
func (es *EventSystem) UnSubscribe(sub *Subscription) error {
	existingSubs := es.eventBus.Topics()
	for _, topic := range existingSubs {
		if topic == sub.event {
			isEmpty, err := es.eventBus.UnSubscribe(sub.event, sub.eventCh)
			if err != nil {
				return err
			}
			if isEmpty {
...
func (es *EventSystem) eventLoop() {
	for {
		...
		case f := <-es.install:
			...
			close(f.installed)
		case f := <-es.uninstall:
			es.logger.Info("not close event ,id = %s",f.id)
			//es.indexMux.Lock()
...
func (es *EventSystem) consumeEvents() {
	for {
		for rpcResp := range es.tmWSClient.ResponsesCh {
			...
			es.indexMux.RUnlock()
			if !ok {
				es.logger.Error("channel for subscription not found", "topic", ev.Query)
				es.logger.Error("list of available channels", "channels", es.eventBus.Topics())
				continue
			}
